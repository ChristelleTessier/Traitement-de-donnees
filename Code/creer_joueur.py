...
def creer_joueur(*, id=None, prenom=None, nom=None, data=None, info = None):
    ...
    spécifier le nom de la variable lors de l'appel de la fonction
    (keyword-only arguments).

    Args:
        id (str, optione):
            L'identifiant unique du joueur. Defaults à None.
        prenom (str, optional):
            Le prénom du joueur. Defaults à None.
        nom (str, optionel):
            Le nom de famille du joueur. Defaults à None.
        info (tuple, optionnel):
            data(dataFrame des joueurs) et sexe(str 'H' ou 'F')

    Returns:
        Joueur ou None:
            Un objet Joueur si les informations sont trouvées, sinon None.
    """
