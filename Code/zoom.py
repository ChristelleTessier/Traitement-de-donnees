...
def zoom_annee(data):
    """
    Permet à l'utilisateur de filtrer les données en fonction de plusieurs
    années sélectionnées.

    Paramètres:
        data (pd.DataFrame):
            Le DataFrame contenant une colonne 'annee'.

    Retourne:
        pd.DataFrame:
            Un sous-ensemble du DataFrame original filtré sur
            les années choisies.
    """
    ...
    while ajout_annee != '0' or valide != '1':
        ...
        print(
            "Vous souhaitez des informations sur l'annee : "
        ...
        if ajout_annee == '0':
            ...
            for annee in annees:
                ...
                data_restriction = pd.concat(
                    [data_restriction, data_temp],
                    axis=0
                    )
...
def zoom_level(data):
    """
    Permet à l'utilisateur de filtrer les données selon un ou plusieurs
    niveaux de tournoi.

    Paramètres:
        data (pd.DataFrame):
            Le DataFrame contenant une colonne 'tourney_level'.

    Retourne:
        pd.DataFrame:
            Un sous-ensemble du DataFrame original filtré par
            niveaux de tournoi.
    """
    ...
    while ajout_level != '0' or valide != '1':
        ...
        texte_liste = [
            f" {k+1} : {level}," for k, level in enumerate(level_liste)
            ]
        texte = "".join(texte_liste) + ".\nIndiquer l'indice en réponse : "
        ...
        while True:
            ...
            try:
                ...
                print(
                    "Erreur : Veuillez entrer indice entre 1 et "
                    f"{len(level_liste)} (un nombre entier)."
                    )

        # Validation du level
        print(
            "Vous souhaitez des informations sur le tournoi de niveau :"
            f" {level_liste[indice_level-1]} ?"
            )
        valide = boucle_01()
        if valide == "1":
...
def zoom_surface(data):
    ...
    Retourne:
        pd.DataFrame:
            Un sous-ensemble du DataFrame original filtré par surface de jeu.
    """
    ...
    while ajout_surface != '0' or valide != '1':
        ...
        texte_liste = [
            f" {k+1} : {surface}," for k, surface in enumerate(surface_liste)
            ]
        texte = "".join(texte_liste) + ".\nIndiquer l'indice en réponse : "
        ...
        while True:
            ...
            try:
                ...
                print(
                    "Erreur : Veuillez entrer indice entre 1 et "
                    f"{len(surface_liste)} (un nombre entier)."
                    )

        # Validation de la surface
        print(
            f"Vous souhaitez des informations sur le tournoi de niveau :"
            f" {surface_liste[indice_surface-1]} ?"
            )
        valide = boucle_01()
        if valide == "1":
...
def zoomer_tab_tournoi(joueur, victoire):
    """
    Filtre les résultats d'un joueur selon différents critères : année, niveau
    de tournoi et surface.

    Paramètres:
        joueur (Joueur):
            L'objet représentant un joueur.
        victoire (bool):
            Si True, filtre les victoires. Sinon, affiche tous les résultats.

    Retourne:
        pd.DataFrame:
            Le DataFrame des résultats filtrés selon les options choisies.
    """
...
def chercher_parcours(joueur, data):
    ...
    Paramètres:
        joueur (Joueur):
            L'objet représentant un joueur.
        data (pd.DataFrame):
            Les résultats de tournois à parcourir (filtrés ou non).

    Retourne:
        pd.DataFrame or None:
            Le DataFrame des matchs dans le tournoi sélectionné ou None
              si aucune sélection.
    """
...
def zoom_graph(data):
    """
    Permet à l'utilisateur de zoomer sur une période spécifique (entre deux
    années) pour les données de classement.

    Paramètres:
        data (pd.DataFrame):
            Le DataFrame contenant une colonne 'ranking_date' (de type
            datetime ou convertible).

    Retourne:
        pd.DataFrame:
            Le DataFrame filtré entre deux années sélectionnées.
    """
